use axum::{extract::State, extract::Path, Json, response::IntoResponse};
...
pub async fn get_books(State(state): State<AppState>) -> impl IntoResponse {
    match find_books(state.db.clone()).await {
        ...
        Err(err) => Err((axum::http::StatusCode::INTERNAL_SERVER_ERROR, Json(BookError::Generic(BookGenericException { message: err.to_string() })))),
    }
}

pub async fn get_book_by_id(State(state): State<AppState>, Path(book_id): Path<i32>) -> impl IntoResponse {
...
pub async fn add_book(State(state): State<AppState>, Json(payload): Json<CreateBook>) -> impl IntoResponse {
